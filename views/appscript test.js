  ...
  try {
    ...
    const destSheet = destDoc.getSheetByName(destSheetName);
      ...
      for (let periodCount = 1; periodCount <= (periodCountMax - 1); periodCount++) {
        ...
        periodLabelArray.push(`Période ${periodCount}`);
      }
...
for (let i = 0; i < 3; i++) {
  ...
  const dataseanceData = [seanceArray[i], numerodegroupeArray[i], nombredelevesArray[i], niveauArray[i], localArray[i], dateArray[i], periodeArray[i], disponibiliteArray[i]];
  ...
  // Loop through periodeArray and transfer values to columns T and U in Data sheet
  for (let j = 0; j < periodeArray.length; j++) {
    for (let k = 0; k < periodLabelArray.length; k++) {
      if (periodeArray[j].values === periodLabelArray[k]) {
        sourceSheet.appendRow([periodeArray[j], startTimeArray[k], endTimeArray[k]]);
        break; // Exit inner loop once values are transferred
      }
    }
  }
}
